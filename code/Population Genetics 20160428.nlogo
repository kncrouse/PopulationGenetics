...
phenotypes-own [ parent-student first-allele second-allele ]
...
to update-alleles [ one two three four ]
  ...
  repeat one [
    hatch-alleles 1 [
      ...
      set shape "circle"
      set allele-type "one"
      set color read-from-string allele-one-color
  ...
  repeat two [
    hatch-alleles 1 [
      ...
      set shape "circle"
      set allele-type "two"
      set color read-from-string allele-two-color
      set allele-list lput self allele-list
    ]
  ]
  repeat three [
    hatch-alleles 1 [
      ...
      set color read-from-string allele-three-color
      set allele-list lput self allele-list
    ]
  ]
  repeat four [
    hatch-alleles 1 [
      ...
      set color read-from-string allele-four-color
      set allele-list lput self allele-list
    ]
  ]

  while [length allele-list > 1 and length phenotype-group < individuals-per-group ] [
    hatch-phenotypes 1
    [
      let pPhenotype self
...
to execute-reproduce
  ...
  foreach old-phenotype-group [
    ask ? [ reproduce ( one-of total-phenotype-group ) ]
  ]
  foreach old-phenotype-group [ ask ? [remove-phenotype ]]
end
...
to execute-command [command]
  ...
  let aOne count alleles with [ member? parent-phenotype [phenotype-group] of myself and allele-type = "one" ]
  let aTwo count alleles with [ member? parent-phenotype [phenotype-group] of myself and allele-type = "two" ]
  let aThree count alleles with [ member? parent-phenotype [phenotype-group] of myself and allele-type = "three" ]
  let aFour count alleles with [ member? parent-phenotype [phenotype-group] of myself and allele-type = "four" ]

  if command = "initial-count-one"
  [ update-alleles hubnet-message aTwo aThree aFour ]
  if command = "initial-count-two"
  [ update-alleles aOne hubnet-message aThree aFour ]
  if command = "initial-count-three"
  [ update-alleles aOne aTwo hubnet-message aFour ]
  if command = "initial-count-four"
  [ update-alleles aOne aTwo aThree hubnet-message ]
end

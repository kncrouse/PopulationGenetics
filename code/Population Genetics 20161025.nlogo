...
globals [
  ...
  individuals-per-population ; carryng capacity
...
to setup-parameters
  ...
  set individuals-per-population 10
...
to create-phenotype-population
  let parent self
  hatch-phenotypes individuals-per-population
  [
...
to execute-reproduce
  ...
  foreach old-phenotype-population [ ask ? [ reproduce ( one-of total-phenotype-population ) ]]
  foreach old-phenotype-population [ ask ? [ remove-phenotype ]]
end

; student command to update allele frequencies based on input number count
to update-alleles [ one two three four ]

  foreach phenotype-population [ ask ? [ remove-phenotype ]]
  let parent self
  let allele-list []
  ...
  ask alleles with [ parent-phenotype = nobody ] [ die ]
end
...
to phenotype-wander
  let patchy one-of patches in-cone population-radius 60 with [ distance [parent-student] of myself <= population-radius ]
...
to execute-command [command]
  ...
  ; change allele counts of student phenotype-population based on hubnet update
...
to send-info-to-clients
  ...
  if generation-number = 0 [
    hubnet-send user-id "initial-count-one" count alleles with [ member? parent-phenotype [phenotype-population] of myself and allele-type = "one" ]
    hubnet-send user-id "initial-count-two" count alleles with [ member? parent-phenotype [phenotype-population] of myself and allele-type = "two" ]
    hubnet-send user-id "initial-count-three" count alleles with [ member? parent-phenotype [phenotype-population] of myself and allele-type = "three" ]
    hubnet-send user-id "initial-count-four" count alleles with [ member? parent-phenotype [phenotype-population] of myself and allele-type = "four" ]
  ]
...
to update-allele-frequency-plot
  foreach allele-types [
    set-current-plot "Allele Frequencies"
    set-current-plot-pen (word "allele " ?)
    set-plot-pen-color get-allele-color ?
    plot-pen-down
    plotxy generation-number (100 * (count alleles with [ allele-type = ? ] / (count alleles + 0.00001)))
    plot-pen-up
  ]
end
@#$#@#$#@
...
"" ""
PENS
"allele one" 1.0 0 -204336 true "" ";set-plot-pen-color read-from-string allele-one-color plot 100 * (count alleles with [ allele-type = \"one\" ] / (count alleles + 1))"
"allele two" 1.0 0 -204336 true "" ";set-plot-pen-color read-from-string allele-two-color plot 100 * (count alleles with [ allele-type = \"two\" ] / (count alleles + 1))"
"allele three" 1.0 0 -204336 true "" ";set-plot-pen-color read-from-string allele-three-color plot 100 * (count alleles with [ allele-type = \"three\" ] / (count alleles + 1))"
"allele four" 1.0 0 -204336 true "" "; set-plot-pen-color read-from-string allele-four-color plot 100 * (count alleles with [ allele-type = \"four\" ] / (count alleles + 1))"
...
50
reproduce
set generation-number generation-number + 1\nask students [ execute-reproduce ]\nask students [ foreach n-of (length phenotype-population - individuals-per-population) phenotype-population [ ask ? [remove-phenotype] ]]\nupdate-allele-frequency-plot
NIL
1
T
OBSERVER
